 ...
 *
 * Wire format is kept byte-identical to the Python backend so images are
 * interchangeable. Audit §8 rule: any wire-format change bumps the version
 * here AND in backend/modules/base.py.
 *
 * TWO CONTAINER VERSIONS EXIST (must match backend/modules/base.py):
 *
 *   v1 — the fixed 14-byte header used by this file's spatial-LSB text path
 *        (backend PayloadHeader). Its bytes must never shift:
 *     [HEADER 14 bytes][payload]
 *     HEADER: MAGIC "HSTG" (4) | VERSION=1 (1) | FLAGS (1) | LENGTH u32be (4) | CRC32 u32be (4)
 *
 *   v2 — the rich multi-modal container (backend/modules/container.py) used by
 *        the capacity endpoint / encode pipeline. Big-endian header:
 *     MAGIC "HSTG" (4) | VERSION=2 (1) | FLAGS (1) | PAYLOAD_TYPE (1) |
 *     ECC_SCHEME (1) | COMPRESSION_PRESET (1) | RESERVED (1) |
 *     PAYLOAD_SIZE u32 (4) | FILENAME_LEN u16 (2) | MIME_LEN u16 (2) |
 *     SHA-256 (32) | filename (var) | mime (var)
 *     followed by [zlib? -> Reed-Solomon] payload, then the whole container is
 *     AES-256-GCM encrypted. FLAGS: ENCRYPTED 0x01 | COMPRESSED 0x02 | ECC 0x04.
 *     The header records COMPRESSION_PRESET so extraction knows which recovery
 *     parameters (RS n/k, expected BER) to expect.
 *
 ...
 *
 * NOTE: This module currently implements the v1 text path only. Full v2
 * build/parse parity (zlib + Reed-Solomon in the browser) is wired on the
 * Python backend now and is deferred here to the client-encode prompt; the
 * constants + format above are the byte-compat contract that work will follow.
 */

const MAGIC = new Uint8Array([0x48, 0x53, 0x54, 0x47]); // "HSTG"
// Container versions + flags — mirror backend/modules/base.py exactly.
// Exported as the byte-compat contract the client-encode prompt will implement.
export const HEADER_VERSION_V1 = 1; // legacy fixed 14-byte header (this file's text path)
export const HEADER_VERSION_V2 = 2; // rich container.py format (current container)
export const FLAG_ENCRYPTED = 0x01;
export const FLAG_COMPRESSED = 0x02; // v2: payload zlib-compressed
export const FLAG_ECC = 0x04; // v2: payload carries Reed-Solomon ECC
...
function buildHeader(payloadLength: number, flags: number, crc: number): Uint8Array {
  ...
  header.set(MAGIC, 0);
  header[4] = HEADER_VERSION_V1; // this text path is the v1 container (byte-pinned)
  header[5] = flags;
