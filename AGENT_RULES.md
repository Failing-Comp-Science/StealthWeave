...
│   └── pnpm-workspace.yaml, package.json, tsconfig.base.json (replit.md at root)
├── docs/                       generated artifacts only: HOW_IT_WORKS.pdf + generate_pdf.py
│                               (all *.md docs live at repo root — audit rule)
...
- **Compression presets (2026-08-08, calibrated):** the backend models
  ...
  `COMPRESSION_PRESETS.md` + `evaluation/results/compression_report.md`.
...
13. Reference-repo discipline (audit 2026-08-09): only MIT code may be copied
    ...
    BUM16 framing ideas only. Reuse first, check `REFERENCE_REVIEW.md`.
...
| 2026-08-09 | Carrier-preset benchmark + closeout docs | DONE | `evaluation/benchmark_carrier_presets.py` (kept as the harness): PNG/BMP 21,338 B msg / 21,242 B file for ALL carriers (LSB is carrier-independent); JPEG 512×512 textured 29/101/274 msg B (chat_standard/chat_hd/lossless, container +0/+5/+210 B); MP4 3s 20,505 B for all carriers (CRF-independent carrier grid) with measured BER 0.0060/0.0006/0.0000; PNG + BMP lossless round-trips PASS. Wrote `CAPACITY_MODEL.md` + `CARRIER_PRESETS.md`; appended this session's findings to `WORK_AND_FAILURES.md`. Deleted temp scripts (`evaluation/_repro_runtime.py`, `backend/debug_video*.py`, `backend/find_limit.py`). **187 backend tests pass (3×)**; frontend `tsc --noEmit` + `vite build` exit 0; Playwright e2e passes. |
...
| 2026-08-09 | Reference audit + symptoms log | DONE | Deep-dived AlphaSteg (magic-byte `guess_extension_and_media_type`), javid (Hamming(7,4), PNG-DEFLATE embed), BUM16 (RLE→packet→CRC32→16-FSK framing), Wavest, HideUrBits, openstego (already GPL-known); wrote `REFERENCE_REVIEW.md` + linked it from `codebase_and_repo_audit.md` §5. Logged 8 symptoms (evidence + suspected root causes + next steps) in `WORK_AND_FAILURES.md` §7: A1 capacity/fits-check preset mismatch, A2 no backend magic-byte probe, A3 audio carrier gap, A4 single video-stack SPOF, B1 video dies on 2nd-gen re-encode, B2 image robustness inversion (heavy survives/standard fails), B3 video capacity `0` (resolved 08-08), B4 PyAV↔OpenCV collision (mitigated). Added audit constraints here (§4 items 9-12, §5, §7 rules 11-13, §9 items 8-11). No code changes. |
| 2026-08-09 | Collapse UI to the single **LOSSLESS** preset (Phase 2 rename) | DONE | Canonical `UnifiedPresetId` renamed `LOCAL_HIGH_CAPACITY` → **`LOSSLESS`** in `unified_presets.py`; `LOCAL_HIGH_CAPACITY` + `LOSSLESS_HIGH_CAPACITY` kept as legacy aliases → LOSSLESS in `_LEGACY_ALIASES`. `stego.py` gains `_is_lossless_alias()` so pre-rename ids resolve via the unified path on `/capacity` (echo `preset: "LOSSLESS"`) and `/encode` (old clients no longer 400); Form/Query defaults → LOSSLESS. OpenAPI defaults/descriptions + `PresetCapacity.preset_id` enum → LOSSLESS; orval regenerated (api-client-react + zod). Frontend: `UNIFIED_PRESETS` collapsed to one LOSSLESS entry, `DEFAULT_UNIFIED_PRESET="LOSSLESS"`; `encode.tsx` step 04 is now a static "PRESET: Lossless" card (testid `preset-LOSSLESS`); preset RadioGroup, `changePreset`, and preset-change `refetchCapacity` deleted; `capacity-api.ts`/`stego-api.ts` send `preset=LOSSLESS`; e2e updated. Phase 2.4 test: JPEG LOSSLESS capacity ≥ CHAT_HD ≥ CHAT_STANDARD. **221 backend tests pass**; frontend `tsc --noEmit` + `vite build` (PORT=5173 BASE_PATH=/) exit 0. |
...
11. **Resolve the `[TODO]` in `REFERENCE_REVIEW.md` §4.3**: live-crawl the
    HideUrBits app to record its carrier/payload method, then close the
    reference-review open threads.

---

## §12 — Audit 2026-08-09 (findings + implementation plan)

Full evidence-cited writeup: `AUDIT.md` (repo root). Summary and queued work:

### Verified findings
1. **Slow detection = the auto-fired capacity probe, not the classifier.**
   `file-classify.ts` reads 16 header bytes (fast). `encode.tsx` fires
   `analyzeCover()` on cover selection AND on every preset change; for video the
   backend full-decodes the clip 2-3× (`video_capacity._probe_pyav`,
   `keyframe_grid`, `_I_frame_sampled_blocks`) before returning a capacity figure.
2. **UI is already single-preset.** One radio group (`preset-radio-group`,
   step 04). The two-axis residue is the API contract (legacy
   `carrier_preset`/`payload_compression`/`compress`/`compression_preset` form
   fields still accepted), the `compression_preset`-keyed capacity response
   (`capacity-api.ts` remap), and `CARRIER_PRESETS.md`/`COMPRESSION_PRESETS.md`.
3. **All presets report the same capacity by design** for video (CRF-independent
   carrier count) and PNG/BMP (pixel-count-only spatial capacity). Only JPEG
   differs (29/101/274 B).
4. **Video capacity < PNG**: 20,505 B (3 s 1080p MP4) vs 85,818 B (512² PNG) —
   video uses 1/30 frames × 1/64 px × mid-band-only vs PNG's 3 bits/px on every
   pixel. Per-resolution ceiling table in `AUDIT.md` §Phase 4.
5. **"Stuck at 55%"** = `stego-api.ts` sends two fixed 55% events then blocks on
   `fetch` (no streaming, no timeout); backend video closed loop (`MAX_ITERS=8`,
   DELTA_SCHEDULE 6..26) re-encodes+decodes the whole clip per iteration.
6. **Frontend image stego**: `stego.ts` is v1-text-only + orphaned. PNG/BMP LSB
   is feasible client-side (SubtleCrypto + Canvas); needs v2 container
   (zlib + RS(255,223)) port + Web Worker. JPEG DCT-QIM stays backend.
7. **4K blocked by pipeline, not format**: all frames held in RAM (24.9 MB/frame
   → ~7.5 GB for a 10 s 4K clip), Python block loop (129,600 blocks/I-frame),
   full-decode probes, no timeout.

### Implementation plan (next phases; do NOT start without this list)
- **P0-fix:** metadata-only video probe + per-file capacity cache; SSE/polling
  video progress from the closed loop; server + client timeouts with cancel;
  stream video frames (drop all-frames-in-RAM) + vectorize `_count_eligible`;
  per-preset lossless slot model (all-frames/short-GOP/multi-bit video, bpc>1
  PNG); collapse UI to the single Lossless preset.
- **P1-clean:** deprecate legacy preset form fields (clear 400/410) + drop the
  legacy ladder tests; switch capacity response to `preset_id`; complete-or-delete
  `stego.ts` (chosen: complete + wire); pick one `/api/healthz` source; archive
  the two axis docs into `CAPACITY_MODEL.md`.
- **P2-future:** WASM JPEG for full client-side image stego; 4K path with P0
  fixes; re-enable chat presets when a real transmission use case lands.

### Doc moves (this task)
All `.md` moved to repo root per the "all .md at root" rule: `CAPACITY_MODEL.md`,
`CARRIER_PRESETS.md`, `COMPRESSION_PRESETS.md`, `HOW_IT_WORKS.md`,
`REFERENCE_REVIEW.md`, `UNIFIED_PRESETS.md`, `frontend/replit.md` → `replit.md`.
`docs/` now holds only generated artifacts (`HOW_IT_WORKS.pdf`,
`generate_pdf.py`, paths updated). Cross-references updated throughout; verified
no residual `docs/X.md` links.